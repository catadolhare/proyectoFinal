...
.then(dataCharts =>{
    ...
    for(let i=0; i<3; i++){
        recomArtista.innerHTML = `<article class="rec"><a href="detailArtista.html"><img src="${dataCharts.artists.data[i].picture_big}" alt="${dataCharts.artists.data[i].name}"><h3>${dataCharts.artists.data[i].name}</h3></a></article>`
    }
    //Recomendaciones Albumes
    for(let i=0; i<3; i++){
        recomAlbum.innerHTML = `<article class="rec"><a href="detailAlbum.html"><img src="${dataCharts.albums.data[i].cover_big}" alt="${dataCharts.albums.data[i].title}"><h3>${dataCharts.albums.data[i].title}</h3></a></article>`
    }
    //Recomendaciones Cancion
    for(let i=0; i<3; i++){
        recomCancion.innerHTML = `<article class="rec"><a href="detailCancion.html"><img src="${dataCharts.tracks.data[i].artist.picture_big}" alt="${dataCharts.tracks.data[i].artist.name}"><h3>${dataCharts.tracks.data[i].title}</h3></a></article>`
    }
})
